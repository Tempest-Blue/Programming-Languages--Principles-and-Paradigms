...
let rec assoc (d,k,l) = match l with 
  | [] -> d
  | (x,y)::tl -> if (x = k) then y else assoc (d,k,tl);;
 ...
 *  ** your function should be tail recursive **
*)
let rec wwhile (f,b) = let (x,y) = f(b) in
    (if not(y) then x else wwhile(f,x));;
...
let fixpoint (f,b) = wwhile ((let func b = if (b = f b) 
                                then (f b, false) else (f b, true) in func),b);;
...
let rec exprToString e = match e with
  ...
  | Twice (a,b)	    -> "((" ^ exprToString a ^ "-" ^ exprToString b ^ ")/2)"
  | Thrice (a,b,c)    -> "(" ^ exprToString a ^ "+" ^ exprToString b ^ "+" ^ exprToString c ^ ")/3";;
...
let buildTwice(a,b)    		   = Twice(a,b)
let buildThrice(a,b,c)		   = Thrice(a,b,c)
...
let rec eval (e,x,y) = match e with
  ...
  | Twice(a,b)	  -> ((eval(a,x,y)-.eval(b,x,y))/.2.0)
  | Thrice(a,b,c)   -> (eval(a,x,y)+.eval(b,x,y)+.eval(c,x,y))/.3.0

let sampleExpr =
  buildCosine(buildSine(buildTimes(buildCosine(buildAverage(buildCosine(buildX()),buildTimes(buildCosine (buildCosine (buildAverage(buildTimes (buildY(),buildY()),buildCosine (buildX())))),buildCosine (buildTimes (buildSine (buildCosine(buildY())),buildAverage (buildSine (buildX()), buildTimes
...
let rec build (rand, depth) = match depth with
  | 0 -> let x = rand(0,2) in (if (x=0) then buildX() else buildY())
  | d -> let y = rand(0,7) in
        if (y = 0) then buildSine(build(rand,d-1))
        else if (y = 1) then buildCosine(build(rand,d-1))
        else if (y = 2) then buildAverage(build(rand,d-1), build(rand,d-1))
        else if (y = 3) then buildTimes(build(rand,d-1), build(rand,d-1))
        else if (y = 4) then buildTwice(build(rand,d-1), build(rand,d-1))
        else if (y = 5) then buildThrice(build(rand,d-1),build(rand,d-1),build(rand,d-1))
        else buildThresh(build(rand,d-1), build(rand,d-1),build(rand,d-1), build(rand,d-1));;
