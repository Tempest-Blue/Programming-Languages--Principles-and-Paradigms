...
let rec clone x n = match n with
  | 0 -> []
  | m -> if m > 0 then List.append (x::[]) (clone x (n-1)) else [];;
...
let padZero l1 l2 = let x = List.length l1 - List.length l2 in 
  let a = List.append (clone 0 x) l2 in 
  let b = List.append (clone 0 (abs x)) l1 in
    if (x > 0) then (l1,a) else (b,l2);;
...
let rec mulByDigit i l = 
  let mul i l = if (i < 1) then [] else
      let f a x = let y = i*x in
          match a with 
            | [] -> y / 10 :: (y mod 10 :: [])
            | h::t -> (y+h) / 10 :: (y+h) mod 10 :: t
      in
      let base = [] in
...
let bigMul l1 l2 = 
  ...
  let args = List.rev(l2) in
    (*let (_, res) =*) List.fold_left bigAdd [] (List.fold_left f base args) (*in
                                                                               res*) 
