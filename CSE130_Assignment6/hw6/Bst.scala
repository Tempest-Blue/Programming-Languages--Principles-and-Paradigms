...
sealed abstract class BST[A <% Ordered[A]] extends AbstractSet[A] 
{
  ...
  def contains(x: A): Boolean = 
    this match {
      case Leaf()        => false
      case Node(e,l,r)   => x == e || l.contains(x) || r.contains(x)
    }
  ...
  def removeMin : (A, BST[A]) = 
    this match {
      ...
      case Node(e,l,r)      => (l.removeMin._1,Node(e,l.removeMin._2,r))
    }
  
  def remove(x: A): BST[A] = 
    this match {
      case Leaf()           => Leaf()
      case Node(e,l,r)      => if (x < e) {Node (e,l.remove(x),r)}
                               else if (x > e) {Node(e,l,r.remove(x))}
                               else { (l,r) match {
                                case (Leaf(), Leaf()) => Leaf()
                                case (_,Leaf())       => { val (min,tree) = l.removeMin
                                                            Node(min,Leaf(),tree)}
                                case _                => { val (min,tree) = r.removeMin
                                                            Node(min,l,tree)}}}
    }  
}
...
object BST {
  ...
  def build[A <% Ordered[A]](xs: List[A]): BST[A] = {
    if (xs.length == 0) {
      ...
      Node(xs(0), build(xs.filter(_ < xs(0))), build(xs.filter(_>xs(0))))
    }
  }
}
...
object BstProperties extends Properties("BST") {
  ...
  val prop_multiset = forAll((xs: List[Int]) => 
    BST(xs).toList == xs.toSet.toList.sorted
  )
